...
def list_rooms(*, sort_by: str, order: str) -> QuerySet[Room]:
    # Вторичная сортировка по идентификатору делает порядок результатов стабильным.
...
def list_room_bookings(*, room_id: int) -> QuerySet[Booking]:
    # Явно проверяем комнату, чтобы отличить пустой список от неизвестного room_id.
